...
def admin_only(f):
    ...
    def wrapper(*args, **kwargs):
        # if id is not 1, return abort with a 403 error
...
#   author: Mapped[str] = mapped_column(String(250), nullable=False)
    ...
    #blog_comments = relationship("Comment", back_populates="comment_author")
    # Parent relationship
...
class Comment(db.Model):
   ...
   # blog_id : Mapped[int] = mapped_column(Integer, ForeignKey('blog_posts.id'))
    # blog_comment = relationship("BlogPost", back_populates="blog_posts")
    # Child relationship
...
def register():
    ...
    if register_form.validate_on_submit():
        temp_email = request.form['email'].lower()
        result = db.session.execute(db.select(User).where(func.lower(User.email) == temp_email))
        user = result.scalar()
        # Note, email in db is unique so will only have one result.
        if user:
            # User already exists
            flash("This email is already being used. Perhaps you could log in instead.")
...
def login():
    ...
    if login_form.validate_on_submit():
        ...
        result = db.session.execute(db.select(User).where(func.lower(User.email) == user_email.lower()))
        user = result.scalar()
        if not user:
...
if __name__ == "__main__":
    # use debug=True when working in a virtual environment
